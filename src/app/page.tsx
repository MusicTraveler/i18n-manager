...
export default function Home() {
	...
	const {
		...
		error,
		isLoading,
	} = useSWR<Message[], Error>("messages", fetcher);
	...
	const tableData = useMemo(() => {
		...
		for (const row of keyMap.values()) {
			row.missingLocales = allLocales.filter(
				(l) => !row.messagesByLocale.has(l),
			);
		}
		...
		const buildHierarchy = (prefix: string): TableRow[] => {
			...
			const directChildren = new Map<
				string,
				{ keys: string[]; hasChildren: boolean; isPrefixKey: boolean }
			>();
			...
			for (const fullKey of keysWithPrefix) {
				...
				if (!directChildren.has(nextSegment)) {
					directChildren.set(nextSegment, {
						keys: [],
						hasChildren: false,
						isPrefixKey: false,
					});
			...
			for (const [segment, { keys, hasChildren, isPrefixKey }] of Array.from(
				directChildren.entries(),
	...
	const filteredTableData = useMemo(() => {
		const filterNode = (node: TableRow): TableRow | null => {
			...
			if (
				filterKey &&
				node.fullKey &&
				!node.fullKey.toLowerCase().includes(filterKey.toLowerCase())
			) {
				shouldInclude = false;
			}
			...
			if (filterLocale && node.messagesByLocale) {
				const hasMatchingLocale = Array.from(
					node.messagesByLocale.values(),
				).some((m) =>
					m.locale.toLowerCase().includes(filterLocale.toLowerCase()),
				);
			...
			const filteredChildren = node.children
				? node.children
						...
						.filter((child): child is TableRow => child !== null)
				: undefined;
	...
	const completenessStats = useMemo(() => {
		return allLocales.map((locale) => {
			...
			const missingCount = Array.from(allKeysSet).filter(
				(key) => !localeKeys.has(key),
			).length;
			const percentage =
				allKeysSet.size > 0
					? Math.round((localeKeys.size / allKeysSet.size) * 100)
					: 0;
	...
	const handleRowSelect = useCallback(
		(fullKey: string, isSelected: boolean) => {
	...
	const handleDeleteSelected = useCallback(async () => {
		...
		if (
			confirm(`Are you sure you want to delete ${selectedKeys.size} key(s)?`)
		) {
			try {
	...
	const handleDeleteCategory = useCallback(
		async (categoryLabel: string, leafKeys: string[]) => {
	...
	const [lastExpandedCategory, setLastExpandedCategory] = useState<
		string | null
	>(null);
	...
	return (
		<div className="bp6-dark" style={{ minHeight: "100vh" }}>
			...
			<Section
				id="i18n-manager-section"
				suppressHydrationWarning
				style={{ paddingTop: "70px", padding: "70px 20px 20px 20px" }}
			>
				<div style={{ marginBottom: "20px" }}>
					<MessageCompletenessStats
						messages={messages}
					...
					>
						{isLoading ? (
							...
							<NonIdealState
								...
								description={
									messages.length === 0
										? "Add your first message to get started!"
